  ...
  methods: {
    ...
    changeSubmit() { //选择题题库提交
      ...
      this.$axios({ //提交数据到选择题题库表
        url: 'http://47.103.10.220:8008/question/addQuestion',
        method: 'post',
        data: this.$qs.stringify({
          // ...this.postChange          
          'content':this.postChange.question,
          'typeA':this.postChange.answerA,
          'typeB':this.postChange.answerB,
          'typeC':this.postChange.answerC,
          'typeD':this.postChange.answerD,
          'right':this.postChange.rightAnswer,
          'valueStr':this.postChange.value,
          'types':this.optionValue,
        })
      }).then(res => { //添加成功显示提示
