  ...
  <div id="myExam">
    ...
    <div class="wrapper">
      <ul class="top">
        ...
        <li><el-button type="warning" @click="search()">搜索试卷</el-button></li>
      </ul>
      <ul class="paper" v-loading="loading">
        <li class="item" v-for="(item,index) in papers" :key="index">
          <h4 @click="toExamMsg(item.pid)">{{item.pid}}</h4>
          <p class="name">{{item.pid}}-{{item.pname}}</p>
          <div class="info">
            <!-- <i class="el-icon-loading"></i><span>{{item.examDate.substr(0,10)}}</span> -->
            <i class="iconfont icon-icon-time"></i><span v-if="item.totalTime != null">限时{{item.duration}}分钟</span>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.$axios({
          url:'http://47.103.10.220:8008/paper/queryPaper',
          method:'post',
          data:data,
        }).then(res => {
    ...
    //跳转到试卷详情页
    toExamMsg(pid) {
      this.$router.push({path: '/examMsg', query: {pid: pid}})
      console.log(pid)
    }
  }
}
